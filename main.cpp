...
int main(int argc, char** argv) {
    ...
    if (world_rank == 0) {
        ...
        if (TOTAL_NUTS < NUM_SQUIRRELS) {
            // В теории не должно быть, но на всякий случай:
            for (int i = 0; i < world_size; ++i) sendcounts[i] = (i < TOTAL_NUTS ? 1 : 0);
        } else {
            int base_each = 1; // минимум 1
            long long remaining = TOTAL_NUTS - (long long)NUM_SQUIRRELS; // >=0

            // Сгенерируем (world_size - 1) случайных точек в [0, remaining]
    ...
    // Каждый процесс печатает свою строку (чтобы mpirun вывел ровно 100 строк)
    ...
    // Формат вывода строго как ожидается в тестах
